...
public:
	...
	*/
	...
	DoublySortedLinkedList<T>& operator=(const DoublySortedLinkedList<T>& l);

	DoublySortedLinkedList(const DoublySortedLinkedList<T>& l);
};
...
template <typename T>
DoublySortedLinkedList<T>& DoublySortedLinkedList<T>::operator=(const DoublySortedLinkedList<T>& l)
{
	MakeEmpty();

	m_nLength = l.m_nLength;

	m_currentNode = l.m_pFirst->next;
	current = m_pFirst;

	while (m_currentNode->next != NULL)
	{
		m_newNode = new DoublyNodeType<T>;
		m_newNode->data = m_currentNode->data;

		m_newNode->prev = current;
		m_newNode->next = current->next;
		current->next->prev = m_newNode;
		current->next = m_newNode;

		current = current->next;				//현재 노드
		m_currentNode = m_currentNode->next;	//복사할 노드
												//cout << "노드 생성" << endl;
	}
...
template <typename T>
DoublySortedLinkedList<T> ::DoublySortedLinkedList(const DoublySortedLinkedList<T>& l)
{
...
template <typename T>
void DoublySortedLinkedList<T> ::ResetList() {

	current = m_pFirst;
}

template <typename T>
int DoublySortedLinkedList<T> ::Display(T item) {

	bool moreToSearch, found;
	DoublyNodeType<T> * location;

	location = m_pFirst->next;
	found = false;
	moreToSearch = (location != m_pLast);
...
template <typename T>
void DoublySortedLinkedList<T>::GetNextItem(T& item) {
	// current pointer 이 NULL이라면 처음 node를 가리킴.
	if (current == m_pFirst)
	{
		current = m_pFirst->next;
	}
	else
...
{
	DoublyNodeType<T> * test;
	try
	{
		test = new DoublyNodeType<T>;
		delete test;
		return false;
...
int DoublySortedLinkedList<T>::Add(T item)
{
	...
	else // 처음이 아닐 때
	{
		while (1)
		{
			if (item < itor.m_pCurPointer->data || itor.m_pCurPointer == m_pLast) // 맞는 자리를 찾는다.
			{
				DoublyNodeType<T> *pItem = new DoublyNodeType<T>;
				pItem->data = item;
